...
## [0.29.0](https://github.com/SikamikanikoBG/homelab-monitor/releases/tag/v0.29.0) — 2026-08-01 · **The GPU tab, on every box — per-card history, fan speed, and alerts that know the difference between hot and configured**
*Until now the hub got charts and every other machine got a still photograph — not a rendering choice, a storage one. Per-card history is now kept for every host, so one tab serves the whole fleet: a panel per card on a shared scale, fan speed (never collected anywhere before), thermal-throttle windows shaded on the sparkline, and which card each service is actually sitting on. Plus alerts that stay quiet about a card doing exactly what you configured it to do.*
